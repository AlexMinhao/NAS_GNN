...
class SimpleNASController(torch.nn.Module):
    ...
    def __init__(self, args, search_space, action_list, controller_hid=100, cuda=True, mode="train",
        ...
        self.controller_hid = controller_hid  #100
        self.is_cuda = cuda
        ...
        self.encoder = torch.nn.Embedding(num_total_tokens, controller_hid)  # 53*K -> 100
        ...
        for key in self.search_space:
            ...
            decoder = torch.nn.Linear(controller_hid, size)  #100 -> 53
    ...
    def forward(self,
        ...
        hx, cx = self.lstm(embed, hidden)  #hidden tuple:2  * [1 100]  -> hx [1,100] cx -> [1,100]
        logits = self._decoders[action_name](hx) # attention_type ([1, 7])
    ...
    def sample(self, batch_size=1, with_details=False):
        ...
        inputs = torch.zeros([batch_size, self.controller_hid]) # batch_size([1, self.controller_hid 100])
        ...
        for block_idx, action_name in enumerate(self.action_list):
            ...
            logits, hidden = self.forward(inputs,              #Train [1, 100]   derive [100,100]   logits: [100,53]
                                          hidden,
                                          action_name,
        ...
        dags = self._construct_action(actions) #[['gat', 'max', 'tanh', 1, 128, 'cos', 'sum', 'tanh', 4, 16]]
    ...
    def init_hidden(self, batch_size):
        zeros = torch.zeros(batch_size, self.controller_hid)  #64 100
