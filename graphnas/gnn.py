...
class GraphNet(torch.nn.Module):
    ...
    def evalate_actions(self, actions, state_num):
        state_length = len(actions)  #10  ['gat', 'max', 'tanh', 1, 128, 'cos', 'sum', 'tanh', 4, 6]
        if state_length % state_num != 0:
            raise RuntimeError("Wrong Input: unmatchable input")
        layer_nums = state_length // state_num # 10 / 5 = 2
    ...
    def evaluate_structure(self, actions, layer_nums, state_num=6):
        ...
        for i in range(layer_nums): # 2
            head_num = actions[i * state_num + 3]  #['gat', 'max', 'tanh', 1, 128, 'cos', 'sum', 'tanh', 4, 6]
            out_channels = actions[i * state_num + 4]
            hidden_units_list.append(head_num * out_channels) #[128, 24]
            out_channels_list.append(out_channels) # [128, 6]
