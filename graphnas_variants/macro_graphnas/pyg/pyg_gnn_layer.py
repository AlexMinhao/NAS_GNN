...
class GeoLayer(MessagePassing):
    ...
    def forward(self, x, edge_index):
        """"""
        edge_index, _ = remove_self_loops(edge_index) # torch.Size([2, 9104])
        edge_index, _ = add_self_loops(edge_index, num_nodes=x.size(0)) #torch.Size([2, 12431])
        # prepare
    ...
    def message(self, x_i, x_j, edge_index, num_nodes):
        ...
        if self.att_type == "const":
            ...
            alpha = self.apply_attention(edge_index, num_nodes, x_i, x_j) # [12431, 1])    [12431, 4]
            alpha = softmax(alpha, edge_index[0], num_nodes)
            # Sample attention coefficients stochastically.
            if self.training and self.dropout > 0:
                alpha = F.dropout(alpha, p=self.dropout, training=True)

            neighbor = x_j * alpha.view(-1, self.heads, 1) # 1: neighbor: x_j ([12431, 1, 128]) *  alpha([12431, 1, 1]) = ([12431, 1, 128])
        if self.pool_dim > 0:                              # 2: neighbor: x_j ([12431, 4, 6]) *  alpha([12431, 4, 1]) = ([12431, 4, 6])
            for layer in self.pool_layer:
    ...
    def apply_attention(self, edge_index, num_nodes, x_i, x_j):
        if self.att_type == "gat":
            alpha = (torch.cat([x_i, x_j], dim=-1) * self.att).sum(dim=-1) # torch.Size([12431, 1])
            alpha = F.leaky_relu(alpha, self.negative_slope)
                        # x_i ([12431, 1, 128])   x_j ([12431, 1, 128])    self.att ([1, 1, 256])
        ...
        elif self.att_type == "cos":  #self.att = ([1, 4, 12])
            wl = self.att[:, :, :self.out_channels]  # weight left  torch.Size([1, 4, 6])
            wr = self.att[:, :, self.out_channels:]  # weight right torch.Size([1, 4, 6])
            alpha = x_i * wl * x_j * wr #([12431, 4, 6]) * ([1, 4, 6]) * ([12431, 4, 6]) * ([1, 4, 6]) = ([12431, 4, 6])
            alpha = alpha.sum(dim=-1) # torch.Size([12431, 4])
    ...
    def update(self, aggr_out): # 1: torch.Size([3327, 1, 128]) 2: torch.Size([3327, 4, 6])
        if self.concat is True:
            aggr_out = aggr_out.view(-1, self.heads * self.out_channels) # torch.Size([3327, 128])
        else:
            aggr_out = aggr_out.mean(dim=1)  # 1: torch.Size([3327, 128]) 2: torch.Size([3327, 6])
