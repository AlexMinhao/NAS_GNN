...
# num_label   6
        ...
        for i in range(layer_nums):
            ...
            self.acts.append(act_map(act)) # tanh tanh
            if self.residual:
                if concat:
    ...
    def forward(self, x, edge_index_all): # .edge_index_all ([2, 9104])
        output = x  # x-> torch.Size([3327, 3703])
        if self.residual:
            ...
            for i, (act, layer) in enumerate(zip(self.acts, self.layers)): # activation function   GeoLayer
                output = F.dropout(output, p=self.dropout, training=self.training)
                if self.batch_normal:
                    output = self.bns[i](output)
                output = act(layer(output, edge_index_all)) #torch.Size([3327, 6])
