...
def sparse_mx_to_torch_sparse_tensor(sparse_mx):
    ...
    sparse_mx = sparse_mx.tocoo().astype(np.float32) # 2 9104 ？
...
def load_citation(dataset_str="cora", normalization="FirstOrderGCN", cuda=True, gamma=1, degree=2, L=None, K=None):
    ...
    for i in range(len(names)):
        with open("../graphnas_variants/data/{}/raw/ind.{}.{}".format(dataset_str, dataset_str.lower(), names[i]), 'rb') as f:
    ...
    test_idx_reorder = parse_index_file("../graphnas_variants/data/{}/raw/ind.{}.test.index".format(dataset_str,dataset_str))
    ...
    adj = nx.adjacency_matrix(nx.from_dict_of_lists(graph))  # (3327, 3327)
